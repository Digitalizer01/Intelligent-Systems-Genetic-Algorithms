...
import algoritmos.Individuo_Bukin;
import algoritmos.Individuo_Bukin;
import algoritmos.Individuo_Bukin;
import algoritmos.Individuo_Bukin;
import algoritmos.Poblacion_Bukin;
import main.SwingWorkerRealTime;

/**
 * Clase que ejecuta la gestion del algoritmo genetico de la funcion Bukin.
 * 
 *
 */
public class EjecucionBukin {
	...
	public static void main(String[] args) throws Exception {
		...
		swingWorkerRealTime_mejor_fitness.go("Mejor fitness", "Bukin", "Iteración", "Valor fitness");

		SwingWorkerRealTime swingWorkerRealTime_fitness_total = new SwingWorkerRealTime();
		swingWorkerRealTime_fitness_total.go("Fitness total", "Bukin", "Iteración", "Valor fitness");

		SwingWorkerRealTime swingWorkerRealTime_mejores_genes_x = new SwingWorkerRealTime();
		swingWorkerRealTime_mejores_genes_x.go("Mejores genes - X", "Bukin", "Iteración", "Valor X");

		SwingWorkerRealTime swingWorkerRealTime_mejores_genes_y = new SwingWorkerRealTime();
		swingWorkerRealTime_mejores_genes_y.go("Mejores genes - Y", "Bukin", "Iteración", "Valor Y");
		...
		Poblacion_Bukin pop = new Poblacion_Bukin(ELITISM_K, POP_SIZE, MAX_ITER, MUTATION_RATE, CROSSOVER_RATE);
		Individuo_Bukin[] newPop = new Individuo_Bukin[POP_SIZE];
		// ****************
		newPop = pop.getPopulation();
		// ***************
		Individuo_Bukin[] indiv = new Individuo_Bukin[2];

		// current population
		System.out.println("Current population:");
		System.out.print("Total Fitness = " + pop.gettotalFirness());
		System.out.println(" ; Best Fitness = " + pop.findBestIndividuo_Bukin().getFitnessValue());
		System.out.print("Best Individual: ");
		pop.findBestIndividuo_Bukin().show_Individual();
		...
		for (int iter = 0; iter < MAX_ITER && acabar == 0; iter++) {
			...
			LinkedList<Individuo_Bukin> lista_mejores_individuos = new LinkedList<>();

			Individuo_Bukin[] individuos_hijos = new Individuo_Bukin[2];

			// Elitism
			for (int i = 0; i < ELITISM_K; ++i) {
				newPop[count] = pop.findBestIndividuo_Bukin_varios(lista_mejores_individuos);
			...
			while (count + 1 < POP_SIZE) {
				...
				if (pop.getm_rand() < CROSSOVER_RATE) {
					...
					for (int i = 0; i < individuos_hijos.length; i++) {
						Individuo_Bukin individuo_Bukin = individuos_hijos[i];
						if (i == 0) {
							// Hijo 1
							hijo_1_valido = individuo_Bukin.individuo_valido();
						} else {
							// Hijo 2
							hijo_2_valido = individuo_Bukin.individuo_valido();
						}
					}
					...
					if (hijo_1_valido == true || hijo_2_valido == true) {
						...
						if (hijo_1_valido == true || hijo_2_valido == true) {
							...
							case "Estacionario":
								...
								Individuo_Bukin[] individuos_orden;
								if (hijo_1_valido == true && hijo_2_valido == true) {
									individuos_orden = new Individuo_Bukin[4];
									...
									if (hijo_1_valido == false && hijo_2_valido == true) {
										individuos_orden = new Individuo_Bukin[3];
										individuos_orden[0] = indiv[0];
										individuos_orden[1] = indiv[1];
...
//										}
			...
			swingWorkerRealTime_mejores_genes_x.getMySwingWorker()
					.insertar_valor(pop.findBestIndividuo_Bukin().getGene(0));
			swingWorkerRealTime_mejores_genes_y.getMySwingWorker()

					.insertar_valor(pop.findBestIndividuo_Bukin().getGene(1));

			swingWorkerRealTime_mejor_fitness.getMySwingWorker()
					.insertar_valor(pop.findBestIndividuo_Bukin().getFitnessValue());
			swingWorkerRealTime_fitness_total.getMySwingWorker().insertar_valor(pop.gettotalFirness());

			if (pop.findBestIndividuo_Bukin().getFitnessValue() == 0.0) {
		...
		// best indiv
		Individuo_Bukin bestIndiv = pop.findBestIndividuo_Bukin();
