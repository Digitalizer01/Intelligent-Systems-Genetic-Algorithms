...
/* 
* Clase encargada de gestionar un individuo referido al algoritmo Ackley.
* 
*
*/
public class Individuo_Ackley {
	 ...
	 */
	public Individuo_Ackley(double[] genes, double fitnessValue) {
		this.genes = genes;
	 ...
	 */
	public Individuo_Ackley(Individuo_Ackley individuo) {
	...
	public double evaluate() {
		...
		for (int i = 0; i < genes.length; i++) {
			x += Math.pow(genes[i], 2);
			y += (Math.cos(2 * Math.PI * genes[i]));
		}

		value = -20.0 * Math.exp(-0.2 * Math.sqrt(0.5 * x)) - Math.exp(0.5 * y) + Math.exp(1.0) + 20;
		;
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		Individuo_Ackley other = (Individuo_Ackley) obj;
