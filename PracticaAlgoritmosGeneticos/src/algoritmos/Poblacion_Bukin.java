...
public class Poblacion_Bukin {
	...
	private Individuo_Bukin[] m_population;
	 ...
	 */
	public Poblacion_Bukin(int eLITISM_K, int pOP_SIZE, int mAX_ITER, double mUTATION_RATE, double cROSSOVER_RATE) {
		ELITISM_K = eLITISM_K;
		POP_SIZE = pOP_SIZE;
		MAX_ITER = mAX_ITER;
		...
		m_population = new Individuo_Bukin[POP_SIZE];

		// init population
		for (int i = 0; i < POP_SIZE; i++) {
			m_population[i] = new Individuo_Bukin();
	 ...
	 */
	public void setPopulation(Individuo_Bukin[] newPop) {
	 ...
	 */
	public Individuo_Bukin[] getPopulation() {
	 ...
	 */
	public Individuo_Bukin findBestIndividuo_Bukin() {
	 ...
	 */
	public Individuo_Bukin findBestIndividuo_Bukin_varios(
			LinkedList<Individuo_Bukin> lista_mejores_individuos) {
		int idxMax = 0, idxMin = 0;
		double currentMax = 0.0;
		double currentMin = 1.0;
		double currentVal;
	 ...
	 * @return Individuo.
	 */
	public Individuo_Bukin metodo_ruleta() {
		...
		Individuo_Bukin[] individuos_ordenados = new Individuo_Bukin[this.POP_SIZE];

		for (int i = 0; i < individuos_ordenados.length; i++) {
			individuos_ordenados[i] = new Individuo_Bukin(this.m_population[i]);
		}
		...
		for (int i = 0; i < individuos_ordenados.length - 1; i++) {
			for (int j = 0; j < individuos_ordenados.length - i - 1; j++) {
				if (Math.abs(individuos_ordenados[j + 1].getFitnessValue()) < Math
					...
					Individuo_Bukin x = individuos_ordenados[j + 1];
...
//		}
		...
		// values
		// de cada individuo.
		Individuo_Bukin[] individuos_ordenados_transformacion = new Individuo_Bukin[this.POP_SIZE];

		for (int i = 0; i < individuos_ordenados.length; i++) {
			individuos_ordenados_transformacion[i] = new Individuo_Bukin(individuos_ordenados[i]);
		}
//
...
//		System.out.println("idx: " + idx);
	 ...
	 * @return Individuo.
	 */
	public Individuo_Bukin metodo_rank() {
		// Ordenamos los individuos en base a su fitness value.
		// De mejor a peor (mejor: mas cercano a 0)
...
//		System.out.println("idx: " + idx);
	 ...
	 */
	private void imprimir_individuos_fv(Individuo_Bukin[] poblacion) {
	 ...
	 * @return Individuo.
	 */
	public Individuo_Bukin[] seleccion_competitiva(int n) {
...
//		imprimir_individuos_fv();
		...
		Individuo_Bukin[] poblacion_individuos = new Individuo_Bukin[this.POP_SIZE];

		for (int i = 0; i < poblacion_individuos.length; i++) {
			poblacion_individuos[i] = new Individuo_Bukin(this.m_population[i]);
		}
...
//		}
		...
		Individuo_Bukin individuo_mejor_fitness = new Individuo_Bukin();
...
//		}
		...
		Individuo_Bukin individuo_mejor_fitness_2 = new Individuo_Bukin();
...
//		imprimir_individuos_fv();
		...
		Individuo_Bukin[] array_individuos = new Individuo_Bukin[2];
	 ...
	 * @return Individuo.
	 */
	public Individuo_Bukin seleccion_truncada(int n, int cantidad_eliminar) {
		// Tenemos por ejemplo un numero de 50 individuos de la poblacion
...
//		}
		...
		Individuo_Bukin[] individuos_aleatorios = new Individuo_Bukin[array_aleatorios.length];

		for (int i = 0; i < array_aleatorios.length; i++) {
			individuos_aleatorios[i] = poblacion_individuos[array_aleatorios[i]];
		}

		Individuo_Bukin temp = new Individuo_Bukin();
...
//		imprimir_individuos_fv(individuos_aleatorios);
		...
		Individuo_Bukin[] aux = new Individuo_Bukin[individuos_aleatorios.length - cantidad_eliminar];
...
//		imprimir_individuos_fv(aux);
	 ...
	 * @return Individuo.
	 */
	public static Individuo_Bukin[] crossover(Individuo_Bukin indiv1, Individuo_Bukin indiv2) {
		Individuo_Bukin[] newIndiv = new Individuo_Bukin[2];
		newIndiv[0] = new Individuo_Bukin();
		newIndiv[1] = new Individuo_Bukin();

		int randPoint = m_rand.nextInt(Individuo_Bukin.SIZE);
		int i;
		for (i = 0; i < randPoint; ++i) {
			newIndiv[0].setGene(i, indiv1.getGene(i));
			newIndiv[1].setGene(i, indiv2.getGene(i));
		}
		for (; i < Individuo_Bukin.SIZE; ++i) {
	 ...
	 * @return Individuo.
	 */
	public static Individuo_Bukin[] cruce_en_un_punto(Individuo_Bukin indiv1, Individuo_Bukin indiv2) {
...
//		indiv2.show_Individual();
		...
		Individuo_Bukin indiv_hijo1;
		Individuo_Bukin indiv_hijo2;

		Individuo_Bukin[] hijos = new Individuo_Bukin[2];
...
//			System.out.println("Dividimos desde la primera seccion.");
			...
			indiv_hijo1 = new Individuo_Bukin(genes_hijo1, -1);
			indiv_hijo1.evaluate();

			indiv_hijo2 = new Individuo_Bukin(genes_hijo2, -1);
...
//		hijos[1].show_Individual();
	 ...
	 * @return Individuo.
	 */
	public static Individuo_Bukin[] cruce_uniforme(Individuo_Bukin indiv1, Individuo_Bukin indiv2) {
		int tamanio = indiv1.SIZE;
...
//		indiv_hijo2.show_Individual();
	...
	public static void main(String[] args) {
		Poblacion_Bukin poblacion = new Poblacion_Bukin(0, 10, 1, 1, 1);
		// poblacion.seleccion_truncada(5, 3);
		double[] genes1 = { 111.111, 222.222 };
		Individuo_Bukin indiv1 = new Individuo_Bukin(genes1, -1);
		indiv1.evaluate();

		double[] genes2 = { 444.444, 555.555 };
		Individuo_Bukin indiv2 = new Individuo_Bukin(genes2, -2);
		indiv2.evaluate();
