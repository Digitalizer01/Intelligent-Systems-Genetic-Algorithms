...
import algoritmos.Individuo_Beale;
import algoritmos.Individuo_Beale;
import algoritmos.Individuo_Beale;
import algoritmos.Individuo_Beale;
import algoritmos.Poblacion_Beale;
import main.SwingWorkerRealTime;

/**
 * Clase que ejecuta la gestion del algoritmo genetico de la funcion Beale.
 * 
 *
 */
public class EjecucionBeale {
	...
	public static void main(String[] args) throws Exception {
		...
		swingWorkerRealTime_mejor_fitness.go("Mejor fitness", "Beale", "Iteración", "Valor fitness");

		SwingWorkerRealTime swingWorkerRealTime_fitness_total = new SwingWorkerRealTime();
		swingWorkerRealTime_fitness_total.go("Fitness total", "Beale", "Iteración", "Valor fitness");

		SwingWorkerRealTime swingWorkerRealTime_mejores_genes_x = new SwingWorkerRealTime();
		swingWorkerRealTime_mejores_genes_x.go("Mejores genes - X", "Beale", "Iteración", "Valor X");

		SwingWorkerRealTime swingWorkerRealTime_mejores_genes_y = new SwingWorkerRealTime();
		swingWorkerRealTime_mejores_genes_y.go("Mejores genes - Y", "Beale", "Iteración", "Valor Y");
		...
		Poblacion_Beale pop = new Poblacion_Beale(ELITISM_K, POP_SIZE, MAX_ITER, MUTATION_RATE, CROSSOVER_RATE);
		Individuo_Beale[] newPop = new Individuo_Beale[POP_SIZE];
		// ****************
		newPop = pop.getPopulation();
		// ***************
		Individuo_Beale[] indiv = new Individuo_Beale[2];

		// current population
		System.out.println("Current population:");
		System.out.print("Total Fitness = " + pop.gettotalFirness());
		System.out.println(" ; Best Fitness = " + pop.findBestIndividuo_Beale().getFitnessValue());
		System.out.print("Best Individual: ");
		pop.findBestIndividuo_Beale().show_Individual();
		...
		for (int iter = 0; iter < MAX_ITER && acabar == 0; iter++) {
			...
			LinkedList<Individuo_Beale> lista_mejores_individuos = new LinkedList<>();

			Individuo_Beale[] individuos_hijos = new Individuo_Beale[2];

			// Elitism
			for (int i = 0; i < ELITISM_K; ++i) {
				newPop[count] = pop.findBestIndividuo_Beale_varios(lista_mejores_individuos);
			...
			while (count + 1 < POP_SIZE) {
				...
				if (pop.getm_rand() < CROSSOVER_RATE) {
					...
					for (int i = 0; i < individuos_hijos.length; i++) {
						Individuo_Beale individuo_Beale = individuos_hijos[i];
						if (i == 0) {
							// Hijo 1
							hijo_1_valido = individuo_Beale.individuo_valido();
						} else {
							// Hijo 2
							hijo_2_valido = individuo_Beale.individuo_valido();
						}
					}
					...
					if (hijo_1_valido == true || hijo_2_valido == true) {
						...
						if (hijo_1_valido == true || hijo_2_valido == true) {
							...
							case "Estacionario":
								...
								Individuo_Beale[] individuos_orden;
								if (hijo_1_valido == true && hijo_2_valido == true) {
									individuos_orden = new Individuo_Beale[4];
									...
									if (hijo_1_valido == false && hijo_2_valido == true) {
										individuos_orden = new Individuo_Beale[3];
										individuos_orden[0] = indiv[0];
										individuos_orden[1] = indiv[1];
...
//										}
			...
			swingWorkerRealTime_mejores_genes_x.getMySwingWorker()
					.insertar_valor(pop.findBestIndividuo_Beale().getGene(0));
			swingWorkerRealTime_mejores_genes_y.getMySwingWorker()
					.insertar_valor(pop.findBestIndividuo_Beale().getGene(1));

			pop.findBestIndividuo_Beale().show_Individual();

			swingWorkerRealTime_mejor_fitness.getMySwingWorker()
					.insertar_valor(pop.findBestIndividuo_Beale().getFitnessValue());
			swingWorkerRealTime_fitness_total.getMySwingWorker().insertar_valor(pop.gettotalFirness());

			if (pop.findBestIndividuo_Beale().getFitnessValue() == 0.0) {
		...
		// best indiv
		Individuo_Beale bestIndiv = pop.findBestIndividuo_Beale();
